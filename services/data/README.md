...
相場データ・ポートフォリオ照会用の REST API。BrokerClient 抽象化により moomoo OpenD / kabuステーションを透過的に切替可能。
...
```
Client → [Bearer認証] → FastAPI (:8000) → BrokerClient → OpenD / kabuステーション
```

- `src/broker/factory.py` の `create_broker(BROKER_TYPE)` でブローカーを生成
...
| GET | `/orderbook/{code}` | 要 | 板情報 | moomoo のみ |
...
| 変数 | 必須 | デフォルト | 説明 |
|------|------|-----------|------|
| `KABU_API_PASSWORD` | Yes | - | kabuステーション API パスワード |
| `KABU_HOST` | No | `host.docker.internal` | kabuステーションホスト |
| `KABU_PORT` | No | `18080` | kabuステーションポート |

## 依存関係

- `src/broker/` - BrokerClient Protocol + 各ブローカー実装
- `src/client.py` - MoomooClient (moomoo ブローカー時)
